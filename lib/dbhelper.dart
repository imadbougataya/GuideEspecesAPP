...
class DatabaseHelper {
  ...
  static const String serverUrl = '$baseServerURL/api/marine-species/all-familles-for-order/';
  ...
  Future<void> fetchFamilleDataIfNeeded(String tableName) async {
    ...
    if (connectivityResult == ConnectivityResult.none) {
      logger.d("No internet connectivity for table $tableName. Will not fetch data from API.");
    } else {
      logger.d("Internet connectivity found for table $tableName. Attempting to fetch data from API.");
      await _fetchFamilleDataFromAPIWithRetry(tableName);
    }
  }

  Future<void> _fetchFamilleDataFromAPIWithRetry(String tableName, {int retries = 5}) async {
    int attempt = 0;
    bool success = false;
    while (attempt < retries && !success) {
      ...
      try {
        ...
        success = true;
        logger.d("Data fetched successfully from API for table $tableName on attempt $attempt");
      } catch (e) {
        if (e is TimeoutException || e is SocketException) {
          logger.e("Attempt $attempt for table $tableName failed due to network error: $e");
        } else {
      ...
      if (!success && attempt < retries) {
        logger.d("Retrying to fetch data for table $tableName. Attempt: $attempt");
      }
    }
    if (!success) {
      logger.e("Max retries reached. Failed to fetch data from API for table $tableName.");
    }
  }

  Future<void> _fetchFamilleDataFromAPI(String tableName) async {
    String tableUrl = tableName == "osseux" ? "poissons osseux" : tableName;
    var fullUrl = Uri.parse('$serverUrl$tableUrl');
    logger.d('Request URL: $fullUrl');
    try {
      var response = await http.get(fullUrl).timeout(const Duration(seconds: 100));
      ...
      if (response.statusCode == 200) {
        ...
        logger.d("Data for table $tableName fetched and inserted into database successfully.");
      } else {
        throw Exception('Failed to fetch data for table $tableName from API, status code: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> _insertJsonData(String tableName, List<Map<String, dynamic>> jsonData) async {
    logger.d("_insertJsonData data for table $tableName.");
    ...
    logger.d("${results.length} rows inserted into table $tableName successfully.");
  }

  Future<List<Map<String, dynamic>>> getFamillesTableData(String tableName) async {
    logger.d("Getting getFamillesTableData for table $tableName.");
    await fetchFamilleDataIfNeeded(tableName);
    final db = await database;
    List<Map<String, dynamic>> result = await db.query(tableName.replaceAll(' ', '-'));
    if (result.isEmpty) {
  ...
  Future<List<Map<String, dynamic>>> getEspecesTableData(String groupe, String famille) async {
    final db = await database;
    String tableName = groupe == "poissons osseux" ? "osseux" : "osseux";
    logger.d("getEspecesTableData for table $tableName.");
